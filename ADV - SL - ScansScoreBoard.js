        ...
        function onRequest_entry(context) {
			...
			scansSearch.run().each(function(result) {
				...
				for(var i = 0; i < pickJobs.length; i++){
					...
					if(pickJobs[i] == myLine.values['GROUP(custrecord_adv_picktaskline_pickjob)'][0].text){
						doesContain = true;
						break;
					}
				}
				...
				if(parseInt(myLine.values['SUM(formulanumeric)_1']) == 0){
					...
					if(!doesContain){
					log.audit('myLine.values[MAX(CUSTRECORD_RFS_PTL_ORDERID.internalid]',myLine.values['MAX(CUSTRECORD_RFS_PTL_ORDERID.internalid']);
					...
					pickStatus.push();
					palletJobsRem[myLine.values['GROUP(custrecord_adv_picktaskline_pickjob)'][0].text] = myLine.values['SUM(formulanumeric)_1'];
				...
				return true;
					...
					log.audit('salesOrdersIDs',salesOrdersIDs);
			if(pickJobs.length > 0){
			var completedScans = CompletedScansSearch([1, 9], pickJobs);
			...
			var htmlText = '<html><head><meta http-equiv="refresh" content="' + 30 + ';URL= '+ "'https://5050497.extforms.netsuite.com/app/site/hosting/scriptlet.nl?script=7956&deploy=1&compid=5050497&h=875c495e36e0ce0ae61b"+'" /><title>Pick Job Scoreboard, Pallet- 12th + E-Com</title></head><body style="background-color:#c2d6d6;"> ' +
			'<div class="container" >' +
				...
				'<table cellpadding="5" cellspacing="0" width="100%" height="10%" align="center"><tr><td align="center"><p style="font-size:50px">Pick Job Scoreboard, Pallet- 12th + E-Com</p></td></tr></table>' +
				'<table cellpadding="0" cellspacing="0" width="100%" align="center" vertical-align="middle" style="border: 3px solid black;">' +
					'<tbody>' +
						...
						'		<p style="font-size:25px">Pick Job</p>' +
							'</td>' +
						'	<td align="center" width="40%" style="background-color:#ffffff; border-bottom: 3px solid black; border-right: 3px solid black;">' +
						'		<p style="font-size:20px">Pick Tasks</p>'+
							'</td>' +
						...
						'		<p style="font-size:25px">Customer</p>'+
							'</td>' +
						'	<td align="center" width="10%" style="background-color:#ffffff; border-bottom: 3px solid black; border-right: 3px solid black;">' +
						'		<p style="font-size:25px">Pick Tasls Completed</p>'+
							'</td>' +
						'	<td align="center" width="10%" style="background-color:#ffffff; border-bottom: 3px solid black; border-right: 3px solid black;">' +
						...
						for(var i = 0; i < pickJobs.length; i++){
							...
							if(pickJobs[i] == '- None -'){
								...
								';border-right: 3px solid black;";><p style="font-size:25px; ">'+ pickJobs[i] + '</p></td>';
								
							}else{
								htmlText += '<tr><td align="center"  style="background-color:' + backgroundColor + 
								';border-right: 3px solid black;";><p style="font-size:25px; "><a style="color: black;" href="https://5050497.app.netsuite.com/app/common/custom/custrecordentry.nl?rectype=1519&id=' + pickJobs[i] +'">' + pickJobs[i] + '</a></p></td>';
								
							}
							htmlText += '<td align="center"  style="background-color:' + backgroundColor + 
								';border-right: 3px solid black;";><p style="font-size:20px; ">';
							var splitTasks = pickTasks[pickJobs[i]].split(',');
							...
							for(var x = 0; x < splitTasks.length; x++){
								...
								if(palletJobsDone[pickJobs[i]]){
									htmlText += '<td align="center"  style="background-color:' + backgroundColor + 
									';border-right: 3px solid black;";><p style="font-size:25px; ">' + palletJobsDone[pickJobs[i]] + '</p></td>';
								}
								else{
									htmlText += '<td align="center"  style="background-color:' + backgroundColor + 
									';border-right: 3px solid black;";><p style="font-size:25px; ">' + 0 + '</p></td>';
								}
								htmlText += '<td align="center"   style="background-color:' + backgroundColor + 
								';border-right: 3px solid black;";><p style="font-size:25px; ">' + palletJobsRem[pickJobs[i]] + '</p></td>';
					...
					'</div>' +
					'Reset time: 30 seconds | Monitor 1 | Last Update:'+ (parseInt(currentDate.getMonth()) + 1) + '/' + currentDate.getDate() + '/' + currentDate.getFullYear()  + ' ' + currentDate.getHours() + ':' + currentDate.getMinutes()
			'</body></html>';
